...
int main(int argc, char ** argv)
{
    ...
    // force dimension code
    mtsForceDimension * device = new mtsForceDimension("MTMR");
    device->Configure();
    componentManager->AddComponent(device);
    mtsROSBridge * rosBridgeDevice = new mtsROSBridge("ForceDimensionBridge", rosPeriod, true);
    std::string deviceName = "omega";
    rosBridgeDevice->AddPublisherFromCommandRead<prmPositionCartesianGet, geometry_msgs::PoseStamped>
        ("Device", "GetPositionCartesian",
         "/force_dimension/" + deviceName + "/position_cartesian_current");
    rosBridgeDevice->AddPublisherFromCommandRead<prmVelocityCartesianGet, geometry_msgs::TwistStamped>
        ("Device", "GetVelocityCartesian",
         "/force_dimension/" + deviceName + "/twist_body_current");
    rosBridgeDevice->AddPublisherFromCommandRead<prmForceCartesianGet, geometry_msgs::WrenchStamped>
        ("Device", "GetForceTorqueCartesian",
         "/force_dimension/" + deviceName + "/wrench_body_current");
    rosBridgeDevice->AddPublisherFromCommandRead<double, std_msgs::Float32>
        ("Device", "GetGripperPosition",
         "/force_dimension/" + deviceName + "/gripper_position_current");
    componentManager->AddComponent(rosBridgeDevice);
    componentManager->Connect(rosBridgeDevice->GetName(), "Device",
